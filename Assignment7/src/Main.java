            ...
            switch(MenuOptions.values()[choice - 1]){ //Checking the user input by comparig it to enum constants
	            ...
	            case OPT9:
	            	...
	            	for(Calculable elem : calculables) {
	            		if(elem instanceof Reservation && Month.equalsIgnoreCase( ((Reservation)elem).getReservationMonth())) { // downcasting 
	            			System.out.printf("\nFor reservation ID: %d, Service type: Room booking, Service Cost: %f ", ((Reservation)elem).getCustomerID(), ((Reservation)elem).getCost());
	            			totalIncome += ((Reservation)elem).getCost(); // downcasting to get the total
	            			checkID = ((Reservation) elem).getCustomerID(); // Laundries and Spas with matching ID will have the same month
	            		}
	            		if(elem instanceof Laundry && checkID == ((Laundry) elem).getCustomerID()) {
	            			System.out.printf("\nFor reservation ID: %d, Service type: Laundry, Service Cost: %f ", ((Laundry)elem).getCustomerID(), ((Laundry)elem).getCost());
	            			totalIncome += ((Laundry)elem).getCost();
	            		}
	            		if(elem instanceof Spa && checkID == ((Spa) elem).getCustomerID()) {
	            			System.out.printf("\nFor reservation ID: %d, Service type: Laundry, Service Cost: %f ", ((Spa)elem).getCustomerID(), ((Spa)elem).getCost());
	            			totalIncome += ((Spa)elem).getCost();
	            		}
	            		if(elem instanceof Bills && Month.equalsIgnoreCase( ((Bills)elem).getMonth()) ) totalBills += ((Bills)elem).getCost();
	            		
	            		if(elem instanceof Employees) totalEmployee += ((Employees)elem).getCost(); // dont need to check for month since salary is monthly
	            	}
